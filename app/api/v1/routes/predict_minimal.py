...
def make_minimal_prediction(house_features: Dict[str, Any]) -> float:
    """
    Make a price prediction using only the model's required features.

    This endpoint accepts only the subset of house attributes that the model
    actually needs — callers do not have to supply the full column set from
    the house-sales data.  Demographic features are still enriched on the
    backend via the provided zipcode.

    Args:
        house_features: Dictionary with only the required house features
                        (bedrooms, bathrooms, sqft_living, sqft_lot, floors,
                        sqft_above, sqft_basement) plus zipcode.
